...
class PlateauLRControllerTest(unittest.TestCase):
    ...
    def test_improvement_holds_lr_and_three_bad_epochs_reduce(self):
    ...
    def test_counter_resets_and_reduces_again_after_three_bad_epochs(self):
        ...
        self.assertEqual(result['event'], 'lr_reduced')
    ...
    def test_eta_min_and_metric_fallback(self):
        optimizer, controller = self.make_controller(eta_min=0.095)
        ...
        self.assertAlmostEqual(optimizer.param_groups[0]['lr'], 0.095)
        ...
        self.assertEqual(floor['event'], 'lr_floor')
        self.assertAlmostEqual(optimizer.param_groups[0]['lr'], 0.095)
        self.assertGreaterEqual(optimizer.param_groups[0]['lr'], 0.095)
